...
class Particle {
  ...
  wrap(width: number, height: number): void {
    ...
    else if (this.x < 0) { this.x = width; this.y = Math.random() * height; }
    if (this.y > height) { this.y = 0; this.x = Math.random() * width; }
    else if (this.y < 0) { this.y = height; this.x = Math.random() * width; }
  }
...
class PerlinNoise {
  ...
  constructor(seed: number = 0) { this.seed(seed); }

  private fade(t: number): number { return t * t * t * (t * (t * 6 - 15) + 10); }
  private lerp(a: number, b: number, t: number): number { return (1 - t) * a + t * b; }
    ...
    const step = () => {
      ...
      for (let y = 0; y < rows; y++) {
        ...
        for (let x = 0; x < cols; x++) {
          ...
          const noiseValue = noiseRef.current.perlin3(worldX * scale + timeOffset, worldY * scale, 0);
          flowFieldRef.current[y][x] = noiseValue;
        }
      }
